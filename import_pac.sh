...
mysql -uroot -proot -e"select chr,strand,UPA_start,UPA_end,tot_tagnum,coord from t_"$species"_pac where ref_tagnum>0 into outfile '/var/www/front/tojbrowse/PAC_$species.txt'" $db;
...
/var/www/jbrowse/bin/flatfile-to-json.pl --bed /var/www/front/tojbrowse/PAC_$species.bed --trackLabel "PlantAPA stored PAC" --out /var/www/jbrowse/data/$des/
