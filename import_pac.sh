...
read -p "请输入PA所在的数据库名，物种名，以空格分割 :" db species
...
/var/www/jbrowse/bin/flatfile-to-json.pl --bed /var/www/front/tojbrowse/PAC_$species.bed --trackLabel "System PAC" --out /var/www/jbrowse/data/$des/
jq '(.tracks[-1]) |= . +{"menuTemplate" : [{"url" : "http://localhost/front/sequence_detail.php?species='$species'&seq=AT1G01020","iconClass" : "digitIconDatabase","action" : "newwindow","label" : "go PAC","title" : "go PAC"}]}' /var/www/jbrowse/data/$des/trackList.json > /var/www/jbrowse/data/$des/trackList1.json
