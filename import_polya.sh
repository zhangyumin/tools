...
read -p "请输入PA所在的表名，列名，以空格分割 :" table name
#mysql -uroot -proot -e"select chr,strand,coord,$name from $table" $db
mysql -uroot -proot -e"select chr,strand,coord,$name from $table where $name>0 into outfile '/var/www/front/tojbrowse/$name.txt'" db_server;
...
chmod 777 -R /var/www/jbrowse/data/$species/
echo -n "=====================Step7: move bw files=================================\n";
mv /var/www/front/tojbrowse/$name.PosPA.bw /var/www/jbrowse/data/$species/$name.PosPA.bw
mv /var/www/front/tojbrowse/$name.NegPA.bw /var/www/jbrowse/data/$species/$name.NegPA.bw
echo -n "=====================Step8: use jq to update trackList.json===============\n";
jq '(.tracks) |= . + [{"storeClass" : "JBrowse/Store/SeqFeature/BigWig","urlTemplate" : "./'$name'.PosPA.bw","type" : "JBrowse/View/Track/Wiggle/XYPlot","label" : "'$name' PAT plus strand","key" : "'$name'  PAT plus strand","autoscale" : "local","style" : {"neg_color" : "red","pos_color" : "blue"},"variance_ban" : false}]' /var/www/jbrowse/data/$species/trackList.json > /var/www/jbrowse/data/$species/trackList1.json
jq '(.tracks) |= . + [{"storeClass" : "JBrowse/Store/SeqFeature/BigWig","urlTemplate" : "./'$name'.NegPA.bw","type" : "JBrowse/View/Track/Wiggle/XYPlot","label" : "'$name'  PAT minus strand","key" : "'$name' PAT minus strand","autoscale" : "local","style" : {"neg_color" : "blue","pos_color" : "red"},"variance_ban" : false}]' /var/www/jbrowse/data/$species/trackList1.json > /var/www/jbrowse/data/$species/trackList2.json
rm -rf /var/www/jbrowse/data/$species/trackList.json
rm -rf /var/www/jbrowse/data/$species/trackList1.json
mv /var/www/jbrowse/data/$species/trackList2.json /var/www/jbrowse/data/$species/trackList.json
